...
import { alertsSeed, auditSeed, reviewSeed } from "@/lib/mock/data";
...
function DashboardPage() {
  ...
  const criticas = alertsSeed.filter((a) => a.severidad === "alta");
  const enCola = reviewSeed.filter((r) => r.estado === "En cola");
  const pendientes = criticas.length + enCola.length + proximos.length;
    ...
    <div>
      ...
      <div className="mx-auto max-w-[1440px] space-y-10 px-4 pb-16 sm:px-6 lg:px-10">
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-3 xl:grid-cols-6">
          <MetricCard label="Colaboradores" value={employees.length} tone="neutral" caption="Activos en la nómina vigente" />
          <MetricCard label="Por vencer" value={proximos.length} tone="medium" caption="Contratos en próximos 30 días" />
          <MetricCard label="Alertas críticas" value={criticas.length} tone="high" caption="Requieren acción jurídica" />
          <MetricCard label="Disciplinarios" value={1} tone="medium" caption="Procesos activos en curso" />
          <MetricCard label="Docs por revisar" value={3} tone="neutral" caption="Extracciones pendientes de validación" />
          <MetricCard label="Aprobaciones IA" value={enCola.length} tone="primary" caption="Sugerencias en cola de revisión" />
        </div>
        ...
        {isEmpty ? (
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            <Panel title="Alertas de compliance" icon={<AlertOctagon className="h-4 w-4" />} action={<Link to="/alertas" className="text-xs text-primary hover:underline">Ver alertas</Link>}>
              <ul className="divide-y divide-border">
                {alertsSeed.slice(0, 4).map((a) => (
                  <li key={a.id} className="px-5 py-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="truncate text-sm text-foreground">{a.motivo}</p>
                        <p className="mt-0.5 text-xs text-muted-foreground">{a.empleado}</p>
                      </div>
            ...
            <Panel title="Actividad reciente" icon={<Activity className="h-4 w-4" />} action={<Link to="/auditoria" className="text-xs text-primary hover:underline">Auditoría</Link>}>
              <ul className="divide-y divide-border">
                {auditSeed.slice(0, 5).map((a) => (
                  <li key={a.id} className="px-5 py-4">
                    <p className="text-sm text-foreground">{a.accion}</p>
                    <p className="mt-1 text-xs text-muted-foreground">{a.usuario} · {a.fecha}</p>
                  </li>
                ))}
              </ul>
            </Panel>
